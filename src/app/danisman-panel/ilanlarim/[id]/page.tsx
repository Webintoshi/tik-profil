...
export default function EditConsultantListingPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
        ...
        try {
            ...
            if (data.success && data.listing) {
                ...
                setForm({
                    ...
                    grossArea: listing.features?.grossArea?.toString() || "",
                    netArea: listing.features?.netArea?.toString() || "",
                    roomCount: listing.features?.roomCount || "",
                    floor: listing.features?.floor?.toString() || "",
                    totalFloors: listing.features?.totalFloors?.toString() || "",
                    buildingAge: listing.features?.buildingAge?.toString() || "",
                ...
                if (listing.images && Array.isArray(listing.images)) {
                    const galleryImages: GalleryImage[] = listing.images.map((img, idx) => {
                        const id = `img-${idx}-${Date.now()}`;
                        if (typeof img === 'string') {
                            return { id, url: img, order: idx, isMain: idx === 0 };
                        }
                        return { id, url: img.url, order: img.order || idx, isMain: img.isMain || idx === 0 };
                    });
    ...
    if (sessionLoading || !session || fetching) {
        return (
            ...
            <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
                <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500">İlan bilgilerini güncelleyin</p>
                    </div>
                </div>
            </header>
            ...
            <main className="max-w-4xl mx-auto p-4">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <motion.section
                        ...
                        className="p-6 rounded-2xl bg-white shadow-sm"
                    >
                        <h2 className="text-lg font-semibold mb-4 text-gray-900">💰 Fiyat</h2>
                    ...
                    <motion.section
                        ...
                        className="p-6 rounded-2xl bg-white shadow-sm"
                    >
                        <h2 className="text-lg font-semibold mb-4 text-gray-900">📋 Durum</h2>
