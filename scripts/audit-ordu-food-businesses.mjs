...
try {
    const totals = await db.query(`
            SELECT
                ...
                count(*) FILTER (WHERE discovery.claim_state = 'unclaimed')::int AS unclaimed,
    ...
    const categories = await db.query(`
            SELECT business.industry_label AS category, count(*)::int AS count
            FROM businesses business
            WHERE business.source = 'google_places_verified_import'
              AND lower(COALESCE(business.city, '')) = 'ordu'
              AND lower(COALESCE(business.industry_id, '')) = 'fastfood'
            GROUP BY business.industry_label
            ORDER BY business.industry_label
        `);
    ...
    const samples = await db.query(`
            SELECT business.slug, business.name, business.industry_label AS category,
                   business.district, NULLIF(BTRIM(business.logo), '') IS NOT NULL AS "hasPhoto"
            FROM businesses business
            WHERE business.source = 'google_places_verified_import'
              AND lower(COALESCE(business.city, '')) = 'ordu'
              AND lower(COALESCE(business.industry_id, '')) = 'fastfood'
