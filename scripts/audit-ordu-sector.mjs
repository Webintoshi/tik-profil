...
export async function auditSectorBusinesses({ sectorKey, connectionString, db: providedDb } = {}) {
    ...
    if (!providedDb && !resolvedConnectionString) throw new Error("DATABASE_URL_or_POSTGRES_URL_required");

    const aliases = SECTOR_ALIASES[sectorKey] ?? [sectorKey];
    const db = providedDb || new pg.Client({ connectionString: resolvedConnectionString });
