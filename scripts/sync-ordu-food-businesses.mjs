...
export const FOOD_CATEGORIES = {
    burger: {
        label: "Burger",
        queryTerms: ["burger", "hamburger"],
        primaryTypes: new Set(["hamburger_restaurant"]),
        namePattern: /(?:burger|hamburger)/i,
    },
    pizza: {
        label: "Pizza",
        queryTerms: ["pizza", "pizzacı"],
        primaryTypes: new Set(["pizza_restaurant", "pizza_delivery"]),
        namePattern: /(?:pizza|pizzacı|pizzeria)/i,
    },
    doner: {
        label: "Döner",
        queryTerms: ["döner", "dönerci"],
        primaryTypes: new Set(["gyro_restaurant", "shawarma_restaurant"]),
        namePattern: /(?:döner|doner|dönerci|shawarma)/i,
    },
    fastfood: {
        label: "Fast Food",
        queryTerms: ["fast food", "büfe", "tost", "tantuni", "kokoreç", "çiğ köfte", "sandviç"],
        primaryTypes: new Set([
            "fast_food_restaurant", "hot_dog_restaurant", "hot_dog_stand", "sandwich_shop",
            "snack_bar", "chicken_restaurant", "chicken_wings_restaurant",
        ]),
        namePattern: /(?:fast\s*food|büfe|bufe|tost|sandviç|sandvic|tantuni|kokoreç|kokorec|çiğ\s*köfte|cig\s*kofte)/i,
    },
};
const QUERY_TERMS = Object.entries(FOOD_CATEGORIES).flatMap(([category, definition]) => (
    definition.queryTerms.map((term) => ({ category, term }))
));
...
const GENERIC_FOOD_TYPES = new Set(["restaurant", "meal_takeaway", "meal_delivery", "food_court", "kebab_shop"]);
const EXCLUDED_TYPES = new Set([
    "grocery_store", "market", "supermarket", "convenience_store", "butcher_shop", "food_store",
]);
const EXCLUDED_NAME_PATTERN = /(?:market|kasap|toptan|fabrika|imalat|malzemeleri|ekipmanları)/i;
...
export async function removeReplaceableImportedFoodBusinesses(client) {
    const result = await client.query(`
...
export function classifyFoodBusiness(place) {
    const name = place?.displayName?.text ?? "";
    const primaryType = place?.primaryType ?? "";
    if (!name || EXCLUDED_TYPES.has(primaryType) || EXCLUDED_NAME_PATTERN.test(name)) return null;
    for (const category of ["burger", "pizza", "doner"]) {
        if (FOOD_CATEGORIES[category].primaryTypes.has(primaryType)) return category;
    }
    for (const category of ["burger", "pizza", "doner"]) {
        if (FOOD_CATEGORIES[category].namePattern.test(name)) return category;
    }
    if (FOOD_CATEGORIES.fastfood.primaryTypes.has(primaryType)) return "fastfood";
    if (primaryType && !GENERIC_FOOD_TYPES.has(primaryType)) return null;
    if (FOOD_CATEGORIES.fastfood.namePattern.test(name)) return "fastfood";
    return null;
}

export function isFoodSearchResult(place) {
    return Boolean(classifyFoodBusiness(place));
}
...
async function discoverPlaces(apiKey) {
    const tasks = ORDU_DISTRICTS.flatMap((district) => (
        QUERY_TERMS.map(({ category, term }) => ({ category, district, term }))
    ));
    ...
    async function worker() {
        while (cursor < tasks.length) {
            ...
            for (const place of places) {
                if (discovered.has(place.id) || !isFoodSearchResult(place)) continue;
                const district = resolveDistrict(place.formattedAddress);
                if (!district) continue;
                discovered.set(place.id, { id: place.id, district, requestedCategory: task.category, search: place });
            }
        }
    }
...
async function getPlaceDetails(apiKey, ref) {
    ...
    const normalizedPlace = {
        ...place,
        foodCategory: classifyFoodBusiness({
            displayName: { text: displayName },
            primaryType: place.primaryType ?? ref.search.primaryType,
...
function slugify(value) {
    return normalizeText(value).replace(/\s+/g, "-").replace(/^-|-$/g, "") || "ordu-fast-food";
}
...
export async function upsertPlace(client, place, business, usedSlugs) {
    ...
    const foodCategory = place.foodCategory ?? classifyFoodBusiness({
        displayName: { text: place.displayName },
        primaryType: place.primaryType,
    });
    if (!foodCategory || !FOOD_CATEGORIES[foodCategory]) throw new Error("food_category_required");
    const industryLabel = FOOD_CATEGORIES[foodCategory].label;
    const socialLinks = {
    ...
    await client.query(`
        INSERT INTO businesses (
            ...
            $1, $2, $3, $4, $4, 'active', 'fastfood', $5, NULL,
            $6, $7, $8::jsonb, $9, $10::jsonb, 'Ordu', $11, $12, $13,
            $14, $15, true, 'google_places_verified_import', $16::jsonb, $17, now(), now()
        )
        ON CONFLICT (id) DO UPDATE SET
            ...
            updated_at = now()
    `, [
        businessId, slug, name, phone, industryLabel, place.formattedAddress ?? null, place.googleMapsUri ?? null,
    ...
    await client.query(`
        INSERT INTO business_discovery_profiles (
            ...
            'unclaimed', 'published', jsonb_build_object(
                'sectorKey', 'fastfood',
                'foodCategory', $7::text,
                'categoryLabel', $8::text
            ), now(), now()
        )
    ...
    `, [
        ...
        place.location?.latitude ?? null, place.location?.longitude ?? null, foodCategory, industryLabel,
    ]);
...
async function main() {
    ...
    try {
        const removedForReplacement = replaceUnclaimed
            ? await removeReplaceableImportedFoodBusinesses(db)
            : 0;
        ...
        const summary = {
            ...
            eligibleByCategory: Object.fromEntries(Object.keys(FOOD_CATEGORIES).map((category) => [
                category,
                places.filter((place) => place.foodCategory === category).length,
            ])),
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
    await main().catch((error) => {
        console.error(error instanceof Error ? error.message : "ordu_food_business_sync_failed");
        process.exitCode = 1;
    });
}
