...
const QUERY_TERMS = ["veteriner", "veteriner kliniği", "hayvan hastanesi", "veterinary clinic"];
...
const ACCEPTED_TYPES = new Set(["veterinary_care", "pet_care", "medical_center", "hospital", "doctor", "service"]);
const VETERINARIAN_NAME_PATTERN = /(?:veteriner|veterinary|veterinerlik|hayvan hastanesi|hayvan kliniği|pet kliniği|pet sağlık|\bvet\b)/i;
const EXCLUDED_NAME_PATTERN = /(?:veteriner işleri|veterinerlik fakültesi|laboratuvar|ilçe tarım|tarım ve orman|petshop|pet shop|pet market|akvaryum)/i;
...
export async function removeReplaceableImportedVeterinarians(client) {
    const result = await client.query(`
        WITH replaceable_imports AS MATERIALIZED (
            ...
            INNER JOIN business_discovery_profiles discovery
                    ON discovery.business_id = business.id
            WHERE business.source = 'google_places_verified_import'
              AND lower(COALESCE(business.industry_id, '')) = 'veteriner'
...
export function isVeterinarianSearchResult(place) {
    const name = place?.displayName?.text ?? "";
    if (!name) return false;
    if (EXCLUDED_NAME_PATTERN.test(name)) return false;
    if (place.primaryType === "veterinary_care") return true;
    if (place.primaryType && !ACCEPTED_TYPES.has(place.primaryType)) return false;
    return VETERINARIAN_NAME_PATTERN.test(name);
}
    ...
    async function worker() {
        while (cursor < tasks.length) {
            ...
            for (const place of places) {
                if (discovered.has(place.id) || !isVeterinarianSearchResult(place)) continue;
...
function slugify(value) {
    return normalizeText(value).replace(/\s+/g, "-").replace(/^-|-$/g, "") || "ordu-veteriner";
}
...
export async function upsertPlace(client, place, business, usedSlugs) {
    ...
    await client.query(`
        INSERT INTO businesses (
            ...
            $1, $2, $3, $4, $4, 'active', 'veteriner', 'Veteriner', NULL,
        ...
        ON CONFLICT (id) DO UPDATE SET
            ...
            status = 'active',
            industry_id = 'veteriner',
            industry_label = 'Veteriner',
    ...
    await client.query(`
        INSERT INTO business_discovery_profiles (
            ...
            'unclaimed', 'published', jsonb_build_object('sectorKey', 'veteriner'), now(), now()
        )
...
async function main() {
    ...
    try {
        const removedForReplacement = replaceUnclaimed
            ? await removeReplaceableImportedVeterinarians(db)
            : 0;
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
    await main().catch((error) => {
        console.error(error instanceof Error ? error.message : "ordu_veterinarian_sync_failed");
        process.exitCode = 1;
    });
}
