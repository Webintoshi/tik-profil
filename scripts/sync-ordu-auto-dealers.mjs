...
const DEALER_TYPES = new Set(["car_dealer", "used_car_dealer"]);
const GENERIC_TYPES = new Set(["store", "establishment", "point_of_interest"]);
const EXCLUDED_TYPES = new Set([
    "car_rental", "car_repair", "car_wash", "auto_parts_store", "tire_shop",
    "vehicle_inspection_station", "motorcycle_dealer",
]);
const AUTO_DEALER_NAME_PATTERN = /(?:oto\s*galeri|otomotiv|otomobil|auto\s*(?:center|gallery|galeri)|motors?|motorlu\s*araçlar|araç\s*(?:alım\s*satım|ticaret)|2\.?\s*el\s*(?:araç|otomobil))/i;
const EXCLUDED_NAME_PATTERN = /(?:rent\s*a\s*car|araç\s*kiralama|oto\s*kiralama|ekspertiz|servis|tamir|yedek\s*parça|lastik|oto\s*yıkama|kaporta|boya|sigorta)/i;
...
export async function removeReplaceableImportedAutoDealers(client) {
    const result = await client.query(`
        WITH replaceable_imports AS MATERIALIZED (
            ...
            INNER JOIN business_discovery_profiles discovery
                    ON discovery.business_id = business.id
            WHERE business.source = 'google_places_verified_import'
              AND lower(COALESCE(business.industry_id, '')) = 'oto_galeri'
...
export function isAutoDealerSearchResult(place) {
    const name = place?.displayName?.text ?? "";
    if (!name) return false;
    if (EXCLUDED_TYPES.has(place.primaryType)) return false;
    if (EXCLUDED_NAME_PATTERN.test(name)) return false;
    if (DEALER_TYPES.has(place.primaryType)) return true;
    if (place.primaryType && !GENERIC_TYPES.has(place.primaryType)) return false;
    return AUTO_DEALER_NAME_PATTERN.test(name);
}
    ...
    async function worker() {
        while (cursor < tasks.length) {
            ...
            for (const place of places) {
                if (discovered.has(place.id) || !isAutoDealerSearchResult(place)) continue;
...
function slugify(value) {
    return normalizeText(value).replace(/\s+/g, "-").replace(/^-|-$/g, "") || "ordu-oto-galeri";
}
...
export async function upsertPlace(client, place, business, usedSlugs) {
    ...
    await client.query(`
        INSERT INTO businesses (
            ...
            $1, $2, $3, $4, $4, 'active', 'oto_galeri', 'Oto Galeri', NULL,
    ...
    await client.query(`
        INSERT INTO business_discovery_profiles (
            ...
            'unclaimed', 'published', jsonb_build_object('sectorKey', 'oto_galeri', 'categoryLabel', 'Oto Galeri'), now(), now()
        )
...
async function main() {
    ...
    try {
        const removedForReplacement = replaceUnclaimed
            ? await removeReplaceableImportedAutoDealers(db)
            : 0;
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
    await main().catch((error) => {
            console.error(error instanceof Error ? error.message : "ordu_auto_dealer_sync_failed");
        process.exitCode = 1;
    });
}
